...
int main(int argc, char **argv){
    ...
    for(int n=1; n<=N_ext; n++){
        ...
        for(int j=(N_previous_meas+1); j<=(N_previous_meas+N_meas); j++){
            // INITIAL CONDITIONS
            for(int i=0; i<N; i++)
                ecosystem.x[i] = 0.0; // Initial conditions in zero, since it is linear dynamics and these are the fluctuations
            ...
            for(int time_step = 0;  time_step < num_transient_time_steps; time_step++){
                one_step_euler_maruyama_linearised_kurtz_ecosystem(&ecosystem, dt);
                //t += dt; // We don't actually need time at the moment
            }
            // Apertura file di scrittura
            #ifdef SAVE_TO_BINARY
            snprintf(name_buffer, CHAR_LENGHT, "Linearised_Trajectories_N%d_c%.2f_%s_%.2g_omega%.1f_dt%.2g.bin", N, c, ia_label, sigma, omega, dt);
            fp = my_open_writing_binary_file(name_buffer);
            #endif
            #ifndef SAVE_TO_BINARY
            snprintf(name_buffer, CHAR_LENGHT, "Linearised_Trajectories_N%d_c%.2f_%s_%.2g_omega%.1f_dt%.2g.txt", N, c, ia_label, sigma, omega, dt);
