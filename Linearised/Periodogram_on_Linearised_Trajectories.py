...
def yvonne_prediction(omega, c, sigma, gamma, b):
    mu = sigma * np.sqrt(2.0/np.pi)
    r = 1.0 / (2*c*gamma*sigma**2) * (b + 1j*omega - np.sqrt((b+1j*omega)**2 - 4*c*gamma*sigma**2))
    norm_r = np.abs(r)**2
    phi = norm_r * (2*b + c*mu) / (1 - norm_r*c*sigma**2)
    return phi
...
ax[1].plot(omega, avg_psd, color='orange', lw=2.0, label='average')
# Analytical prediction
fc = float(c)
fsigma = float(sigma)
gamma = -1.0
b = 0.05
phi = yvonne_prediction(omega, fc, fsigma, gamma, b)
ax[1].plot(omega, phi, color='red', lw=1.50, alpha=0.5, ls='dashed', label='analytical')
# Single species results
for i in range(single_species_to_draw):
    colour = cmap(0.2 + (i / (single_species_to_draw - 1)) * 0.8)
    ax[0].plot(time, single_x[i], color=colour, lw=1.0, alpha=0.4)
    ax[1].plot(omega, single_psd[i], color=colour, lw=1.0, alpha=0.4, label=f'species ${i+1}$')
# Time series plot
#ax[0].set_yscale('log')
