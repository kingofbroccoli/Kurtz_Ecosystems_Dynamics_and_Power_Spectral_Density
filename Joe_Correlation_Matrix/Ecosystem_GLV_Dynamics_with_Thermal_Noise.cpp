...
#include "advanced_random_toolbox.h"
#include "gLV_bath_eco_toolbox.h"
...
int main(int argc, char **argv){
    if(argc != 9) {
        fprintf(stderr,
                "Usage: %s N c mu sigma p_A p_M T N_ext\n",
                argv[0]);
        return MY_FAIL;
    }
    ...
    double mu = atof(mu_label) / c;
    char *sigma_label = argv[4];
    double sigma = atof(sigma_label) / sqrt(c);
    char *pA_label = argv[5];
    double p_A = atof(pA_label);
    char *pM_label = argv[6];
    double p_M = atof(pM_label);
    char *T_label = argv[7];
    double T = atof(T_label);
    int N_ext = atoi(argv[8]); 
    ...
    gLV_ecosystem ecosystem;
    initialise_ecosystem(&ecosystem, N, T, lambda);

    // Create the parameter instances
    char ia_label[] = "MixtureTruncGauss";
    MixtureTruncatedGaussParams interaction_params;
    interaction_params.p_A = p_A;
    interaction_params.p_M = p_M;
    interaction_params.mu = mu;
    ...
    interaction_gen.func = mixture_truncated_gaussian;
    ...
    for(int n=1; n<=N_ext; n++){
        ...
        for(int j=(N_previous_meas+1); j<=(N_previous_meas+N_meas); j++){
            ...
            for(int time_step = 0;  time_step < num_transient_time_steps; time_step++){
                one_step_euler_maruyama_gLV_ecosystem_with_demographic_noise(&ecosystem, dt);
                //t += dt; // We don't actually need time at the moment
            }
            // Apertura file di scrittura
            #ifdef SAVE_TO_BINARY
            double dtsam = save_period * dt;
            snprintf(name_buffer, CHAR_LENGHT, "Trajectories_N%d_c%.2f_%s_mu%s_sigma%s_pA_%s_pM_%s_T%s_dt%.2g_dtsam_%.2g.bin", N, c, ia_label, mu_label, sigma_label, pA_label, pM_label, T_label, dt, dtsam);
            fp = my_open_writing_binary_file(name_buffer);
            #endif
            #ifndef SAVE_TO_BINARY
            snprintf(name_buffer, CHAR_LENGHT, "Trajectories_N%d_c%.2f_%s_mu%s_sigma%s_pA_%s_pM_%s_T%s_dt%.2g_dtsam_%.2g.txt", N, c, ia_label, mu_label, sigma_label, pA_label, pM_label, T_label, dt, dtsam);
        ...
        save_ecosystem_to_file(&ecosystem, "Nodes.txt", "Interactions.txt");
