...
if binary_file:
    filename = f"Trajectories_N{N}_c{c}_{ia_label}_{sigma}_omega{volume_omega}_dt{dt}.bin"
...
for i in range(N):
    ...
    if psd_sum is None:
        ...
        first_surviving_species = np.zeros(single_species_to_draw)
    
    # Add to our running sum
    psd_sum += Pxx
    traj_av += time_series 
    if(surviving < single_species_to_draw):
        single_x[surviving] = time_series
        single_psd[surviving] = Pxx
        first_surviving_species[surviving] = mean_abund
    # Increment surviving species
    surviving += 1

    if (i + 1) % 100 == 0:
        print(f"Processed {i + 1} / {N} time series...")

print(f"We have {surviving} surviving species.")
...
# Sort time series to draw

sorted_indexes = np.argsort(first_surviving_species) 
print(first_surviving_species)
print(sorted_indexes)
print(single_x.shape)
single_x = single_x[sorted_indexes]
print(single_x.shape)

single_psd = single_psd[sorted_indexes]
first_surviving_species = first_surviving_species[sorted_indexes]

# Compute the final average PSD across all surviving species
avg_psd = psd_sum / surviving
traj_av /= surviving
...
plot_path = f"Welch_Method_PSD_Trajectories_N{N}_c{c}_{ia_label}_{sigma}_omega{volume_omega}.pdf"
